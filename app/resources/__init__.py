    ...
    def decorated_function(*args, **kwargs):
        ...
        if 'Authorization' in request.headers:
            if request.headers['Authorization'] != '123':
                raise UsoInvalido(TipoErro.ERRO_VALIDACAO.name, status_code=401, payload="Token inválido.")
        else:
            raise UsoInvalido(TipoErro.ERRO_VALIDACAO.name, status_code=401, payload="Cabeçalho de autorização não "
                                                                                     "encontrado.")
